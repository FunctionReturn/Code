...
import { doComplete } from './helper';
import { parseDocument, isStyleSheet, getInnerRange } from './utils';
...
function syntaxHelper(syntax, currentNode, position) {
  if (syntax && !isStyleSheet(syntax)) {
    const currentHtmlNode = currentNode;
    if (currentHtmlNode && currentHtmlNode.close) {
      const innerRange = getInnerRange(currentHtmlNode);
      if (innerRange && innerRange.containsPosition(position)) {
        if (currentHtmlNode.name === 'style') {
          return 'css';
        }
        if (currentHtmlNode.name === 'script') {
          if (
            currentHtmlNode.attributes &&
            currentHtmlNode.attributes.some(
              x =>
...
const enableEmmet = (editor, monaco) => {
  ...
  monaco.languages.registerCompletionItemProvider('html', {
    ...
    provideCompletionItems(model, position) {
      ...
      if (model.languageId === 'html' || isStyleSheet(model.languageId)) {
        const rootNode = parseDocument(model, false);
        if (!rootNode) {
          return;
        }
        ...
        const syntax = syntaxHelper('vue', currentNode, position);
      }

      const s = doComplete(model, position, 'css', {});
