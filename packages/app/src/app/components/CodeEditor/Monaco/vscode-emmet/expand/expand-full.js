(function(global, factory) {
  ...
  class SnippetsRegistry {
    ...
    remove(data) {
      this._registry = this._registry.filter(
        item => item.level !== data && item.store !== data
      );
    }
  ...
  class Node {
    ...
    removeClass(token) {
      ...
      if (this.hasClass(token)) {
        this.setAttribute(
          'class',
          this.classList.filter(name => name !== token).join(' ')
        );
      }
    }
  ...
  function isSpace(code) {
    return isWhiteSpace(code) || code === 10 /* LF */ || code === 13; /* CR */
  }
  ...
  function consumeText(stream) {
    ...
    if (stream.eat(TEXT_START)) {
      ...
      while (!stream.eof()) {
        ...
        if (ch === TEXT_START) {
          ...
          if (ch === TEXT_START || ch === TEXT_END) {
            result +=
              stream.substring(offset, stream.pos - 2) +
      ...
      stream.pos = start;
      throw stream.error(
        `Unable to find closing ${String.fromCharCode(TEXT_END)} for text start`
      );
    }
  ...
  function replaceRanges(str, ranges, value) {
    ...
    for (let i = ranges.length - 1; i >= 0; i--) {
      ...
      if (str.substr(r[0] + r[1], 1) === '@') {
        ...
        const matches = str
          .substr(r[0] + r[1] + 1 + Number(descendingOrder))
          .match(/^(\d+)/);
        if (matches) {
      ...
      str =
        ...
        (typeof value === 'function'
          ? value(str.substr(r[0], r[1]), offset, descendingOrder)
          : value) +
  ...
  function getNumberingRanges(str) {
    return findUnescapedTokens(str || '', numberingToken).reduce(
      (out, range$$1) => {
  ...
  function replaceNumberingRanges(str, ranges, value, count) {
    const replaced = replaceRanges(
      str,
      ranges,
      (token, offset, descendingOrder) => {
        let _value = descendingOrder
  ...
  function insertContent(node, content) {
    ...
    node.walk(
      child => (inserted |= insertContentIntoPlaceholder(child, content))
    );
  ...
  function parse$4(string) {
    ...
    while (!stream.eof()) {
      ...
      if (code === ESCAPE$1) {
        ...
        (field = consumeField(stream, cleanString.length + pos - offset))
      ) {
        fields.push(field);
 ...
 */
  ...
  function formatAttributes(outNode, profile) {
    ...
    return node.attributes
      .map(attr => {
        ...
        if (
          attr.options.boolean ||
          profile.get('booleanAttributes').indexOf(attrName.toLowerCase()) !==
            -1
        ) {
  ...
  const defaultAttrOptions = {
    ...
    secondary: attrs =>
      attrs
        .map(
          attr => (attr.isBoolean ? attr.name : `${attr.name}=${attr.value}`)
        )
        .join(', '),
  };
    ...
    node.attributes.forEach(attr => {
      ...
      if (reId.test(name)) {
        ...
        const isBoolean =
          attr.value == null &&
          (attr.options.boolean ||
            profile.get('booleanAttributes').indexOf(name.toLowerCase()) !==
              -1);
  ...
  function formatNodeValue(node, profile) {
    if (node.value != null && reNl.test(node.value)) {
      ...
      const maxLength = lines.reduce(
        (prev, line) => Math.max(prev, line.length),
        0
      );

      return lines
        .map((line, i) => `${i ? indent : ''}${pad(line, maxLength)} |`)
        .join('\n');
    }
  ...
  function formatNodeValue$1(node, profile) {
    if (node.value != null && reNl$1.test(node.value)) {
      const indent = profile.indent(1);
      return splitByLines(node.value)
        .map((line, i) => `${indent}${i ? ' ' : '|'} ${line}`)
        .join('\n');
    }
  ...
  function consumeArgument(stream) {
    ...
    while (!stream.eof()) {
      stream.eatWhile(isWhiteSpace);
      value =
        consumeNumericValue(stream) ||
        consumeColor(stream) ||
        consumeQuoted$1(stream) ||
        consumeKeywordOrFunction(stream);
  ...
  var cssSnippets = function(snippets) {
    return nest(
      snippets.map(snippet => new CSSSnippet(snippet.key, snippet.value))
    );
  };
  ...
  function nest(snippets) {
    ...
    for (let i = 0, cur, prev; i < snippets.length; i++) {
      ...
      while (stack.length) {
        ...
        if (
          cur.property.indexOf(prev.property) === 0 &&
  ...
  var index$5 = function(tree, registry, options) {
    ...
    options = {
      ...
      unitAliases: Object.assign(
        {},
        defaultOptions$4.unitAliases,
        options && options.unitAliases
      ),
      fuzzySearchMinScore:
        (options && options.fuzzySearchMinScore) ||
        defaultOptions$4.fuzzySearchMinScore,
    };
  ...
  function resolveNumericValue(property, token, formatOptions) {
    if (token.unit) {
      ...
      token.value !== 0 &&
      unitlessProperties.indexOf(property) === -1
    ) {
  ...
  function css(tree, profile, options) {
    ...
    return render(tree, options.field, outNode => {
      ...
      if (node.attributes.length) {
        const fieldValues = node.attributes.map(attr =>
          stringifyAttribute(attr, options)
        );
  ...
  function injectFields(string, values) {
    ...
    if (fieldsAmount) {
      ...
      while (values.length) {
        ...
        fieldsModel.string =
          fieldsModel.string.slice(0, field.location) +
          value +
  ...
  function parse$5(abbr, options) {
    ...
    return abbr.use(
      index$5,
      options.snippets,
      options.format ? options.format.stylesheet : {}
    );
  }
  ...
  var index$8 = function(node, options) {
    ...
    if (!node.repeat && !isRoot$1(node.parent)) {
      ...
      node.name = node.parent.name
        ? resolveImplicitName(node.parent.name)
        : null;
    }
   ...
   * @type {Set}
   */
  const stylesheetSyntaxes = new Set([
    'css',
    'sass',
    'scss',
    'less',
  ...
  function createSnippetsRegistry(syntax, snippets) {
    return snippets instanceof SnippetsRegistry
      ? snippets
      : snippetsRegistryFactory(
          isStylesheet(syntax) ? 'css' : syntax,
          snippets
        );
  }
  ...
  function createOptions(options) {
    ...
    options.snippets = createSnippetsRegistry(
      isStylesheet(options.syntax) ? 'css' : options.syntax,
      options.snippets
    );
