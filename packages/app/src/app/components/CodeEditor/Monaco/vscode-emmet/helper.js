...
export function doComplete(
  ...
  position: Position,
  syntax: string,
  emmetConfig: EmmetConfiguration
): CompletionList {
  if (
  ...
  if (!isStyleSheet(syntax)) {
    if (
      !snippetKeyCache.has(syntax) ||
      !markupSnippetKeysRegex ||
      markupSnippetKeysRegex.length === 0
    ) {
      let registry = customSnippetRegistry[syntax]
        ? customSnippetRegistry[syntax]
        : createSnippetsRegistry(syntax);

      if (!snippetKeyCache.has(syntax)) {
        snippetKeyCache.set(
          syntax,
          registry.all({ type: 'string' }).map(snippet => {
            return snippet.key;
          })
        );
      }
  ...
  let currentLineTillPosition = getCurrentLine(document, position).substr(
    0,
    position.character
  );
  ...
  const createExpandedAbbr = abbr => {
    ...
    if (expandedText) {
      ...
      expandedAbbr.textEdit.text = expandedAbbr.textEdit.newText;
  ...
  if (isStyleSheet(syntax)) {
    let { prefixOptions, abbreviationWithoutPrefix } = splitVendorPrefix(
      abbreviation
    );
    ...
    if (expandedAbbr) {
      let prefixedExpandedText = applyVendorPrefixes(
        expandedText,
        prefixOptions,
        preferences
      );
      expandedAbbr.textEdit = TextEdit.replace(
        abbreviationRange,
        escapeNonTabStopDollar(addFinalTabStop(prefixedExpandedText))
      );
      expandedAbbr.textEdit.text = expandedAbbr.textEdit.newText;
      expandedAbbr.documentation = replaceTabStopsWithCursors(
        prefixedExpandedText
      );
      expandedAbbr.label = removeTabStops(expandedText);
      expandedAbbr.filterText = abbreviation;

      const stylesheetCustomSnippetsKeys = stylesheetCustomSnippetsKeyCache.has(
        syntax
      )
      ...
      if (
        !completionItems.find(
          x => x.textEdit.newText === expandedAbbr.textEdit.newText
        )
      ) {
    ...
    if (
      ...
      (abbreviation === '-' || /^-[wmso]{1,4}-?$/.test(abbreviation))
    ) {
    ...
    let commonlyUsedTagSuggestions = makeSnippetSuggestion(
      commonlyUsedTags,
      currentWord,
      abbreviation,
      abbreviationRange,
    ...
    if (emmetConfig.showAbbreviationSuggestions === true) {
      let abbreviationSuggestions = makeSnippetSuggestion(
        markupSnippetKeys,
        currentWord,
        abbreviation,
        abbreviationRange,
...
function makeSnippetSuggestion(
  ...
  snippets.forEach(snippetKey => {
    ...
    item.textEdit.text = item.textEdit.newText;
...
function getCurrentLine(document: TextDocument, position: Position): string {
  ...
  return document.getLineTokens(
    lineNumber,
    /* inaccurateTokensAcceptable */ false
  )._text;
}
...
export const emmetSnippetField = (index, placeholder) =>
...
): { abbreviation: string, abbreviationRange: Range, filter: string } {
  const currentLine = getCurrentLine(document, position);
  const currentLineTillPosition = currentLine.substr(0, position.column);
  const { pos, filter } = getFilters(currentLineTillPosition, position.column);
...
export function extractAbbreviationFromText(
  text: string
): { abbreviation: string, filter: string } {
  if (!text) {
    return;
  }
...
function isExpandedTextNoise(
  ...
  if (isStyleSheet(syntax)) {
    ...
    return (
      expandedText === `${abbreviation}: \${1}${after}` ||
  ...
  if (
    commonlyUsedTags.indexOf(abbreviation.toLowerCase()) > -1 ||
...
export function getExpandOptions(
  ...
  let filtersFromProfile: string[] =
    profile && profile['filters'] ? profile['filters'].split(',') : [];
  filtersFromProfile = filtersFromProfile.map(filterFromProfile =>
  ...
  if (emmetConfig['preferences']['format.noIndentTags']) {
    if (Array.isArray(emmetConfig['preferences']['format.noIndentTags'])) {
      ...
      typeof emmetConfig['preferences']['format.noIndentTags'] === 'string'
    ) {
      profile['formatSkip'] = emmetConfig['preferences'][
        'format.noIndentTags'
      ].split(',');
    }
  }
  if (emmetConfig['preferences']['format.forceIndentationForTags']) {
    if (
      Array.isArray(
        emmetConfig['preferences']['format.forceIndentationForTags']
      )
    ) {
      profile['formatForce'] =
        emmetConfig['preferences']['format.forceIndentationForTags'];
    } else if (
      typeof emmetConfig['preferences']['format.forceIndentationForTags'] ===
      'string'
    ) {
      profile['formatForce'] = emmetConfig['preferences'][
        'format.forceIndentationForTags'
      ].split(',');
    }
  }
  if (
    emmetConfig['preferences']['profile.allowCompactBoolean'] &&
    typeof emmetConfig['preferences']['profile.allowCompactBoolean'] ===
      'boolean'
  ) {
  ...
  let addons = {};
  if (
    (filter && filter.split(',').find(x => x.trim() === 'bem')) ||
    filtersFromProfile.indexOf('bem') > -1
  ) {
    addons['bem'] = { element: '__' };
    if (emmetConfig['preferences']['bem.elementSeparator']) {
      addons['bem']['element'] =
        emmetConfig['preferences']['bem.elementSeparator'];
    }
    if (emmetConfig['preferences']['bem.modifierSeparator']) {
      addons['bem']['modifier'] =
        emmetConfig['preferences']['bem.modifierSeparator'];
    }
  }
  if (syntax === 'jsx') {
    addons['jsx'] = true;
  ...
  if (
    (filter && filter.split(',').find(x => x.trim() === 'c')) ||
    filtersFromProfile.indexOf('c') > -1
  ) {
  ...
  for (const v in vendorPrefixes) {
    let vendorProperties =
      preferences['css.' + vendorPrefixes[v] + 'Properties'];
    if (vendorProperties == null) {
      preferences['css.' + vendorPrefixes[v] + 'Properties'] =
        defaultVendorProperties[v];
    }
  }
...
function splitVendorPrefix(
  abbreviation: string
): { prefixOptions: string, abbreviationWithoutPrefix: string } {
...
function applyVendorPrefixes(
  ...
  if (vendors == '-') {
    ...
    for (const v in vendorPrefixes) {
      ...
      if (
        vendorProperties &&
        vendorProperties.split(',').find(x => x.trim() === property)
      )
        defaultVendors += v;
    }
  ...
  for (let index = 0; index < vendors.length - 1; index++) {
    prefixedProperty +=
      '-' + vendorPrefixes[vendors[index]] + '-' + expandedProperty + '\n';
  }
...
export function expandAbbreviation(
  ...
  if (isStyleSheet(options['syntax']) && typeof abbreviation === 'string') {
    ...
    expandedText = applyVendorPrefixes(
      expandedText,
      prefixOptions,
      preferences
    );
...
function getProfile(syntax: string, profilesFromSettings: object): any {
  ...
  let profilesConfig = Object.assign(
    {},
    profilesFromFile,
    profilesFromSettings
  );
  ...
  for (let key in options) {
    switch (key) {
      ...
      case 'tag_nl':
        newOptions['format'] =
          options[key] === true || options[key] === false ? options[key] : true;
        break;
      case 'inline_break':
...
export function getEmmetCompletionParticipants(
  document: TextDocument,
  position: Position,
  syntax: string,
  emmetSettings: EmmetConfiguration,
  result: CompletionList
): any {
  return {
    onCssProperty: context => {
      if (context && context.propertyName) {
        const currentresult = doComplete(
          document,
          position,
          syntax,
          emmetSettings
    ...
    onCssPropertyValue: context => {
      if (context && context.propertyValue) {
        ...
        const validAbbreviationWithColon =
          extractedResults.abbreviation ===
            `${context.propertyName}:${context.propertyValue}` &&
          onlyLetters.test(context.propertyValue);
        if (
          validAbbreviationWithColon || // Allows abbreviations like pos:f
